...
const AnswersView = ({allDataIn,noSmokerAnswers, dataNoSmokerSub, user, setShowThankYou, setNoSmokerSub, setHidden, setNoSmokerAnswers}) => {
   ...
   dataNoSmokerSub.relatives = allDataIn.toString()
    const click = async e => {
        ...
        console.log(user)
       await axios.post(`http://localhost:8080/email-builder?user=${JSON.stringify(user)}&dataNoSmokerSub=${JSON.stringify(dataNoSmokerSub)}&allDataIn=${JSON.stringify(allDataIn)}`)
        .then((res) => {
